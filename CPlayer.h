...
public:
	...
	bool raycastBox(vector3D facingVector, vector3D dummyPosition, vector<model> &box, IModel* bulletTracer, const int kboxQuantity, CPlayer myplayer);
	...
	collisionSide SphereToBox3D(float playerX, float playerY, float playerZ, float cubeXLength, float cubeYLength, float cubeZLength, float cubeXPos, float cubeYPos, float cubeZPos, float playerRadius, float playerOldX, float playerOldZ);
