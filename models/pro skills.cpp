...
const float BARREL_DIAMETER = 5.0f; //use of diamiter to calculate random dispersal of flame particals
...
void main()
{
	...
	IModel* barrel =  barrelMesh->CreateModel(-100.0f, 0.0f, 300.0f);
	IModel* barrel2 = barrelMesh->CreateModel(-160.0f, 0.0f, -345.0f);
	IModel* barrel3 = barrelMesh->CreateModel(120.0f, 0.0f, -7.0f);
	IModel* barrel4 = barrelMesh->CreateModel(100.0f, 0.0f, 150.0f);
	...
	container[0] = cargoMesh->CreateModel(-100.0f, 0.0f, 400.0f);
	...
	container[2] = cargoMesh->CreateModel(120.0f, 0.0f, 200.0f);
	container[2]->Scale(19.0f);
	container[2]->RotateLocalY(-37.0f);
	container[2]->SetSkin("CargoGreen.tga");

	container[3] = cargoMesh->CreateModel(220.0f, 0.0f, 0.0f);
	container[3]->Scale(19.0f);
	container[3]->RotateLocalY(90.0f);

	container[4] = cargoMesh->CreateModel(-100.0f, 50.0f, 400.0f);
	container[4]->Scale(19.0f);
	container[4]->RotateLocalY(90.0f);
	container[4]->SetSkin("CargoGreen.tga");
	...
	{
		ammoClip[i].createAmmoClip(myEngine, "ammo.x");
	}
	...
	IModel* lookToDummy = DummyMesh->CreateModel(); // dummy model for partical flames LookAt 

	lookToDummy->AttachToParent(myPlayer.myCamera->cameraDummy); // attach to player to keep flames always facing player
	...
	while (myEngine->IsRunning())
	{
		...
		//Begin Partical Emmition - for Flames and Smoak 
		////////

		barrelFlame->EmitParticle();
		barrelFlame2->EmitParticle();
		barrelFlame3->EmitParticle();
		barrelFlame4->EmitParticle();

		//Reset Particals
		//Used to reset location of the partical when they have reached the randomised hight
		/////////

		barrelFlame->UpdateParticles (lookToDummy, frameTime, BARREL_DIAMETER);
		barrelFlame2->UpdateParticles(lookToDummy, frameTime, BARREL_DIAMETER);
		barrelFlame3->UpdateParticles(lookToDummy, frameTime, BARREL_DIAMETER);
		barrelFlame4->UpdateParticles(lookToDummy, frameTime, BARREL_DIAMETER);
